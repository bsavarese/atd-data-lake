...
To allow flexibility in meeting various data needs, end-user products (whether a database or CSV file dump) depend on specific use-cases, such as:

* **Case 1: Bluetooth Public Information Request (PIR)**
  * Goal: Provide two years of raw Bluetooth data
...
* **Case 2: CoA Bond Corridors Database/Research Partner**
  ...
  * Significance: Ease of ability to support research institution initiatives
  * Outcomes:
...
Online opinions about setting server time zones vary greatly. For those running a global enterprise, there are strong opinions about running systems in Coordinated Universal Time (UTC). But, the only concern for CoA ATD is Central Time Zone.
